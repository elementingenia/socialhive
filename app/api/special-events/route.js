...
export async function PATCH(req) {
  ...
  // BUG-060/BUG-061 class (Iain, 2026-09-18): raising Total Seats (or
  // dropping Unassigned Seats) never re-checked the waitlist -- capacity was
  // only ever recalculated at cancellation time. promoteWaitlist() recomputes
  // available seats fresh and is a no-op when nothing has actually freed up,
  // so it's safe to call unconditionally on every edit rather than trying to
  // detect exactly which field changed.
