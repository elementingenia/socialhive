...
function seatsLabel(seatsRemaining, maxSeats) {
  ...
  return { text: `${seatsRemaining} of ${maxSeats} seats`, color: 'var(--text-dim)' }
}
    ...
    <div style={{
          ...
          <>
            <input
              ...
              style={{
                ...
                borderRadius: '10px', fontSize: '0.9rem', background: 'var(--surface2)',
                marginBottom: '0.5rem', boxSizing: 'border-box', fontFamily: 'inherit',
              }}
            />
            <div style={{ maxHeight: '180px', overflowY: 'auto', border: '1px solid var(--border)', borderRadius: '10px' }}>
              {filtered.slice(0, 30).map(m => (
                <div
                  key={m.id}
                  onClick={() => { setPickedMovie(m); setMovieSearch('') }}
                  style={{
              ...
              {filtered.length === 0 && (
                <div style={{ padding: '0.75rem', color: 'var(--text-dim)', fontSize: '0.85rem' }}>No movies found</div>
              )}
            </div>
          </>
        )}
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <textarea
          ...
          placeholder="e.g. Bring a chair! BYO drinks."
          rows={2}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '0.85rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          ...
          <div style={{ marginTop: '0.2rem', fontSize: '0.78rem', color: seatsColor, fontWeight: 500 }}>
            {seatsText}
            {ev.waitlist_count > 0 && (
              <span style={{ color: 'var(--amber-dark)', marginLeft: '0.5rem' }}>· {ev.waitlist_count} waiting</span>
            )}
          </div>
          ...
          <div style={{ marginTop: 'auto', paddingTop: '0.5rem', display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
            {myStatus === 'confirmed' && (
              <>
                <span style={{ background: '#dcfce7', color: '#15803d', fontSize: '0.75rem', fontWeight: 700, padding: '0.25rem 0.65rem', borderRadius: '20px' }}>✓ Booked</span>
                <button
            ...
            {myStatus === 'waitlist' && (
              <>
                <span style={{ background: '#fef3c7', color: '#d97706', fontSize: '0.75rem', fontWeight: 700, padding: '0.25rem 0.65rem', borderRadius: '20px' }}>⏳ Waitlist</span>
                <button
                  onClick={cancel} disabled={acting}
                  style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '0.3rem 0.65rem', fontSize: '0.78rem', cursor: acting ? 'not-allowed' : 'pointer', color: 'var(--text-dim)' }}
                >
            ...
            {!myStatus && ev.seats_remaining > 0 && (
              <button
                onClick={book} disabled={acting}
                style={{ background: 'var(--teal)', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.4rem 1rem', fontSize: '0.82rem', fontWeight: 600, cursor: acting ? 'not-allowed' : 'pointer', opacity: acting ? 0.6 : 1 }}
              >
                {acting ? '…' : 'Book'}
              </button>
            )}
            {!myStatus && ev.seats_remaining === 0 && (
              <button
                onClick={book} disabled={acting}
                style={{ background: 'var(--amber)', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.4rem 1rem', fontSize: '0.82rem', fontWeight: 600, cursor: acting ? 'not-allowed' : 'pointer', opacity: acting ? 0.6 : 1 }}
              >
                {acting ? '…' : 'Join Waitlist'}
              </button>
            )}
          </div>
        </div>
      ...
      {isAdmin && (
        <div style={{ padding: '0.5rem 1rem', borderTop: '1px solid var(--border)', background: 'var(--surface2)', display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-dim)' }}>
          <span>{ev.confirmed_count} confirmed · {ev.waitlist_count} on waitlist</span>
          <span>{ev.max_seats} seats total</span>
        </div>
      )}
    </div>
  )
    ...
    <div style={{ padding: '1.25rem 1rem 6rem' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '3rem', fontSize: '0.9rem' }}>
          ...
          {member?.is_admin && (
            <div style={{ marginTop: '0.5rem', fontSize: '0.82rem' }}>Use "+ Add" to schedule one.</div>
          )}
        </div>
      ) : (
