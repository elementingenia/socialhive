...
function EventCard({ event, coordinators, myBooking, myWaitlist, waitlistInfo, isAdmin, onOpen, onEdit, onTogglePayment, togglingId, onCloseOutPayments, closingOut, onRemindPayment, remindingId, onToggleRefund, togglingRefundId }) {
    ...
    const transportOwners = [...confirmedBookings, ...waitlistBookings].map(b => {
      ...
      return {
        ...
        party: party.map(p => ({
          ...
          label: p.guest_name || (p.contact_id ? (p.contact?.name || "Resident") : resolveMemberName(p.member, { canManage: true })),
  ...
  // for the fuller explanation. Social keeps its own separate inline
  // reconciliation summary rather than going through that shared panel, so
  // it needs its own copy built from this component's own data.
  function handleExportReconciliation() {
    const rowFor = (b) => ({
      name: b.member ? resolveMemberName(b.member, { canManage: true, fallback: b.member?.username || b.contact?.name || "—" }) : (b.contact?.name || "—"),
      seats: b.seats || 1,
      amount: balancePhrase(b, event, b.seats || 1),
    })
    ...
    <div onClick={blocked ? undefined : onOpen} style={{
      ...
      <div style={{ padding: "0.9rem 1rem" }}>
        ...
        {(() => {
          ...
          const shareUrl = buildShareUrl("/social/events", event.id)
          ...
          const calendarBtn = evWindow && (
            <AddToCalendarButton url={shareUrl} title={event.title} description={event.description}
              location={shareLocation} start={evWindow.start} end={evWindow.end} colour="var(--terracotta)" />
          )
          return (
            <>
              {ecNames.length > 0 ? (
                <EventCoordinators eventId={event.id} eventTitle={event.title} names={ecNames}
                  colour="var(--terracotta)" style={{ marginBottom: "0.2rem" }} stackNames trailing={calendarBtn} />
              ) : calendarBtn ? (
...
export default function SocialEvents() {
  ...
  // build sequence step 1. Read once on mount; deliberately window.location.search
  // rather than next/navigation's useSearchParams so no Suspense boundary is
  // needed on an already-fully-client page (see app/login/page.js for the
  // alternative pattern where that boundary IS needed).
  useEffect(() => {
