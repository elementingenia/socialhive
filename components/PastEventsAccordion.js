...
export default function PastEventsAccordion({ hubType, clubId, colour = "var(--happenings-news)" }) {
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: "0.5rem 0.75rem 0.75rem" }}>
          {events === null ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {events.map(ev => (
                <button key={ev.id}
                  onClick={() => ev.post_id ? router.push(`/happenings-news?post=${ev.post_id}`) : null}
                  disabled={!ev.post_id}
                  style={{
                    ...
                    cursor: ev.post_id ? "pointer" : "default",
                  }}>
                  ...
                  {ev.post_id ? (
                    ...
                    <span style={{ flexShrink: 0, fontSize: "0.7rem", color: "var(--text-dim)" }}>No recap</span>
                  )}
                </button>
              ))}
            </div>
