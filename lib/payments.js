...
// Has a payment reminder already gone out for this booking today, Sydney
// calendar day (2026-09-15, Iain -- migration 106's payment_reminded_at)?
// Drives the attendee-list bell greying out once used, resetting itself
// naturally at Sydney midnight rather than needing a cron to clear it --
// same "compute fresh from a timestamp, never a separate cleared flag"
// approach as lib/notifications.js's isExpiredEventReminder.
export function isRemindedToday(booking) {
  if (!booking?.payment_reminded_at) return false
  return sydneyTodayStr(new Date(booking.payment_reminded_at)) === sydneyTodayStr()
}
