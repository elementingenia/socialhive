...
//
// The surname is everything AFTER the first name, and the initial comes from
// the first letter of that -- not from the last token. This matters for
// hyphenated and apostrophe surnames, where splitting on non-letters yields
// several tokens for what is really one surname:
//
//   "Susan Ellis-Crewe"     -> Susan + [Ellis, Crewe]  -> SusanE   (not SusanC)
//   "Sherrie Heaton-Lindus" -> Sherrie + [Heaton, ...] -> SherrieH (not SherrieL)
//   "Joe O'Hehir"           -> Joe + [O, Hehir]        -> JoeO     (not JoeH)
//
// Caught on 2026-07-29 while renaming live accounts: the old rule would have
// given Susan Ellis-Crewe the username "SusanC", which nobody would guess.
// Widening then walks that same surname token, and once it is exhausted
// continues into the following tokens, so a hyphenated surname still has
// plenty of tie-break room (SusanE -> SusanEl -> SusanEll -> ... -> SusanEllisC).
export function candidates(fullName) {
  const parts = nameParts(fullName)
  if (!parts.length) return []
  const first = parts[0]
  if (parts.length === 1) return [first]           // no surname on file
  const surname = parts.slice(1).join("")          // "Ellis-Crewe" -> "EllisCrewe"
  const out = []
