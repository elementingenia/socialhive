...
// `emoji` is the club's optional icon (clubs.icon); the nav falls back to the
// generic Clubs glyph when it's unset — per the "colour + text, optional
// emoji, no bespoke per-club artwork" decision.
export function clubNavItems(club) {
  if (!club) return []
  const items = [
    { path: "/clubs", label: "Clubs" },
    { path: `/clubs/${club.slug}`, label: club.name, emoji: club.icon || null },
  ]
