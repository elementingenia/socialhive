...
export const HUB_LABELS = { movie: "Show Time", social: "Social Hive", voting: "Voting", special: "Special Events" }

// Display order for the Questions list: Awaiting answer (open + follow-up
// sent, both need someone's attention) first, then Answered, then Finalised
// -- each group oldest-asked first. Applied at the API layer so both the
// "mine" and "answering" boxes use one definition of the ordering.
const STATUS_GROUP = { open: 0, followup: 0, answered: 1, closed: 2 }
export function sortQuestionsForDisplay(rows) {
  return [...rows].sort((a, b) => {
    const ga = STATUS_GROUP[a.status] ?? 99
    const gb = STATUS_GROUP[b.status] ?? 99
...
export async function eventParentOwnerIds(eventId) {
  ...
  return hubOwnerIds(ev.hub_type)   // 'movie' | 'social' | 'special' | ...
}

// Shared notification-routing hierarchy for anything happening ON an event --
// not just Questions. Strict priority, no merging tiers: this event's own
// active EC(s) if any; otherwise the Owner(s) of its parent hub/club; only if
// BOTH are empty does it fall back to every admin, so nothing is ever
// silently unroutable. Reused by primaryAnswererIds (Questions) below and by
// event-level notifications (e.g. a resident self-reporting a payment) so the
// hierarchy can't drift between call sites the way it briefly did here --
// bookings/route.js's mark_payment_submitted used to notify this event's EC
// UNION every admin unconditionally, which meant every admin (Owner or not,
// EC or not) got a "please confirm this payment" alert even when the event
// already had an EC on it. Fixed per Iain, 2026-09-06: "ECs get the
// notifications for their events, Owners get notifications across all events
// in the area/s they own, and admins only get direct notifications pertinent
// to them ... unless there is an absence of Owner/EC."
export async function eventNotifyRecipients(eventId) {
  let ids = await eventECIds(eventId)
  if (ids.length === 0) ids = await eventParentOwnerIds(eventId)
...
export async function primaryAnswererIds(contextType, contextKey) {
  ...
  if (contextType === "general") {
    ...
    ids = await eventNotifyRecipients(contextKey)
...
export async function contextLabel(contextType, contextKey) {
  if (contextType === "general") return "Admins"
...
// Escalation (question-escalation-check cron, 5-day-unanswered safety net)
// notifies EVERY admin regardless of routing -- the whole point is that the
// normal Owner/EC flow has failed. The routed-answerer-only rule above (2026-09-04)
// otherwise hides a question from an admin whose club/hub has its own Owner, so
// without this an escalated admin had a real notification with no way to reach
// the actual thread in the UI (BUG-052, 2026-09-13, Iain: got the 5-day
// notification, "could not see the question without becoming an owner of that
// Group/Club"). Scoped tightly: only questions that have ACTUALLY been escalated
// (escalated_at set) and are still unresolved (open/followup) join an admin's box
// this way -- everything still inside its 5-day window keeps the 2026-09-04
// trust-the-flow behaviour untouched.
function isUnresolvedEscalation(q) {
  return !!q.escalated_at && (q.status === "open" || q.status === "followup")
}
...
export async function answeringBoxQuestions(member) {
  ...
  if (member.is_admin) {
    ...
    for (const q of allQuestions) {
      ...
      if (answerers.includes(member.id) || isUnresolvedEscalation(q)) out.push(q)
    }
    return out
  }
