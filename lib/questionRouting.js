...
export const HUB_LABELS = { movie: "Show Time", social: "Social" }
...
export async function askableCategories() {
  ...
  const { data: cats } = await supabaseAdmin.from("contact_categories")
    .select("id, name, display_order, contact_category_members(contacts(id, member_id, active, members(id, name, hide_name, status)))")
    ...
    if (isCategoryAskable({ ...c, active: true, askable: true }, memberIds)) {
      ...
      out.push({
        ...
        members: rows.filter(r => byId.has(r.member_id))
          .map(r => ({ id: r.members.id, name: r.members.name, hide_name: r.members.hide_name })),
      })
    }
  }
  return out
