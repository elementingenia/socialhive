...
export async function askableCategories() {
  ...
  const out = []
  for (const c of cats || []) {
    const memberIds = await categoryMemberIds(c.id)
    // The query above already filters active+askable; isCategoryAskable
    // re-asserts all three conditions from one shared rule so the gate can't
    // drift if that query is ever loosened.
