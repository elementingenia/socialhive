...
//
// Change Request #3 (2026-09-08, scoped alongside the Surveys build):
// results are visible ONLY to this specific event's own assigned
// Coordinator, never to Admins/Owners generally. Site-wide `is_admin` /
// area-Owner status used to bypass the visibility toggle entirely
// (isAdmin=true short-circuited to `true` here) -- Iain: "Only the
// assigned event coordinator/s should see results. NOT Admins/Owners."
// An Admin/Owner who is not also the named coordinator now falls through
// to the same `results_visibility_*` toggle as any resident. Callers must
// pass `isCoordinator` computed per-event (event.coordinator_id ===
// viewing member.id), not a blanket admin flag.
export function canSeeResults(event, { field, isCoordinator }) {
