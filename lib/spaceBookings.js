...
import { findSpaceConflict, spaceConflictMessage, fmtTime, fetchLocation } from './eventClash.js'
import { closureMessage, sydneyOffsetMinutes, toInstant } from './spaces.js'
import { resolveMemberName } from './memberName.js'
import { isoToSydneyHHMM, isoToSydneyDateStr } from './date.js'
...
//
// exclude_booking_id (Iain, 2026-08-17, editing a booking): when re-checking
// availability for a booking that's already occupying this exact window,
// its own row must not count as a conflict against itself -- threaded
// straight through to checkSpaceAvailability/findSpaceBookingConflict,
// which already supported this for the same reason listAvailableLocations
// itself is called from an editable form now, not just a fresh one.
export async function listAvailableLocations(db, { event_date, event_time, event_end_time, starts_at, ends_at, exclude_booking_id, viewerId, canManage }) {
  ...
  return Promise.all((locations || []).map(async (loc) => {
    const check = await checkSpaceAvailability(db, {
      location: loc, event_date, event_time, event_end_time, starts_at, ends_at, exclude_booking_id, viewerId, canManage,
    })
    return {
...
// ─── PROMOTE TO SHARED EVENT — Book a Space "allow others" toggle ─────────
// Scope: Book_a_Space_Scope_v2.md / Book_a_Space_Technical_Design.md
// (Iain, 2026-08-22). Bridges a private space_bookings row into a real,
// bookable event using the events/bookings/attendees engine every other hub
// already runs on, rather than inventing a second capacity/invite model.
//
// Iain, 2026-08-22: "Yes can be flipped after the fact" -- so this is not
// tied to booking creation. A resident can create a private booking first
// and promote it to shared later, which is exactly what this function does:
// it takes an EXISTING space_bookings row (purpose='private', event_id=NULL)
// and links it to a newly-created events row. Demoting a shared booking back
// to private is NOT handled here -- not asked for, and what happens to any
// attendees who've already booked the event makes that a separate decision,
// left for a future pass if it's ever wanted.

export const SPACE_EVENT_TITLE_MAX = 100

export function validatePromoteToEvent({ title, max_seats, max_seats_per_booking } = {}) {
  const t = (title || '').trim()
  if (!t) return 'Give this a title so others know what they\'re joining'
  if (t.length > SPACE_EVENT_TITLE_MAX) {
    return `Keep the title to ${SPACE_EVENT_TITLE_MAX} characters or fewer (currently ${t.length})`
  }
  const seats = Number(max_seats)
  if (!Number.isInteger(seats) || seats < 1) return 'Set how many seats are available (1 or more)'
  const perBooking = max_seats_per_booking == null || max_seats_per_booking === ''
    ? 4 : Number(max_seats_per_booking)
  if (!Number.isInteger(perBooking) || perBooking < 1) return 'Seats per booking must be 1 or more'
  if (perBooking > seats) return 'Seats per booking can\'t be more than the total seats available'
  return null
}

// db: supabaseAdmin. bookingId: the space_bookings row to promote.
// actorId: the member doing the promoting (must be the original booker or
// an admin -- checked here, not left to the caller, since this writes a new
// events row and a coordinator grant). fields: { title, max_seats,
// max_seats_per_booking, allow_nonresident_guests, require_attendee_names }.
// No Bus fields here (Iain, 2026-08-22, "NO variation from what we already
// have") -- Social only ever offers the Community Bus for offsite events
// (form.location_type === "offsite"), and a promoted Book a Space event is
// always location_type: 'onsite' below, so exposing Bus here would offer
// something Social's own convention never would. Revisit only if Book a
// Space ever grows genuine offsite-gathering support (a materially bigger
// scope item -- flagged to Iain, not built speculatively).
//
// Returns { event_id } on success, or { error, status }.
export async function promoteSpaceBookingToEvent(db, bookingId, actorId, isAdmin, fields) {
  const { data: booking, error: fetchErr } = await db
    .from('space_bookings')
    .select('id, location_id, starts_at, ends_at, purpose, event_id, title, booked_by')
    .eq('id', bookingId).eq('status', 'confirmed').maybeSingle()
  if (fetchErr) throw new Error(`Could not load booking: ${fetchErr.message}`)
  if (!booking) return { error: 'Booking not found', status: 404 }
  if (booking.event_id) return { error: 'This booking is already shared', status: 409 }
  if (!isAdmin && booking.booked_by !== actorId) {
    return { error: 'Only the person who made this booking can share it', status: 403 }
  }

  const validationError = validatePromoteToEvent(fields)
  if (validationError) return { error: validationError, status: 400 }

  const event_date = isoToSydneyDateStr(booking.starts_at)
  const event_time = isoToSydneyHHMM(booking.starts_at)
  const event_end_time = isoToSydneyHHMM(booking.ends_at)

  // events.location is a display copy of the room name -- every other
  // write path (Social/Clubs/Show Time) sets it at write time via
  // fetchLocation(), and the DB trigger (migration 071) only ever
  // cascades a LATER rename, it never backfills a fresh insert. Skipping
  // this left every promoted space event with location_id set but
  // location null -- EventSlideOut's location line has nothing to show,
  // so a specific room (e.g. "Firepit #5") silently disappeared and only
  // the generic "Spaces" hub chip was visible (Iain, 2026-08-23).
  const loc = await fetchLocation(db, booking.location_id)

  const { data: event, error: insertErr } = await db
    .from('events')
    .insert({
      hub_type: 'space',
      title: (fields.title || booking.title || 'Space booking').trim(),
      event_date, event_time, event_end_time,
      location_id: booking.location_id,
      location: loc?.name || null,
      location_type: 'onsite',
      max_seats: Number(fields.max_seats),
      max_seats_per_booking: fields.max_seats_per_booking == null || fields.max_seats_per_booking === ''
        ? 4 : Number(fields.max_seats_per_booking),
      // Same server-side gate Social/Clubs/Show Time all use: these two
      // fields only ever mean anything once a booking can hold more than
      // one seat -- force both false rather than trust whatever the client
      // sent for a single-seat event.
      allow_nonresident_guests: Number(fields.max_seats_per_booking) > 1 ? !!fields.allow_nonresident_guests : false,
      require_attendee_names: Number(fields.max_seats_per_booking) > 1 ? !!fields.require_attendee_names : false,
    })
    .select('id')
    .single()
  if (insertErr) throw new Error(`Could not create event for this booking: ${insertErr.message}`)

  const { error: linkErr } = await db
    .from('space_bookings')
    .update({ event_id: event.id, purpose: 'event' })
    .eq('id', bookingId)
  if (linkErr) throw new Error(`Booking created the event but failed to link back to it: ${linkErr.message}`)

  await db.from('event_coordinators').insert({ event_id: event.id, member_id: actorId, assigned_by: actorId })

  // The organiser gets their own seat automatically -- they created the
  // gathering, they're obviously attending it. payment_status is left at
  // its DB default ('not_required') since Book a Space has no payment
  // tracking (explicitly out of scope, Book_a_Space_Scope_v2.md).
  await db.from('bookings').insert({
    event_id: event.id, member_id: actorId, seats: 1, status: 'confirmed', booked_at: new Date().toISOString(),
  })

  return { event_id: event.id }
}

// Edit an already-shared space event's own details -- scoped deliberately
// narrow (Iain, 2026-08-23: "the creator/owner of the booking" can "modify
// the details"). Same field set PromoteBookingModal collects, editable
// after the fact, rather than growing into a full event-edit form: title,
// seats, and the attendee-naming toggles. Date/time/location are NOT
// editable here -- the underlying space_bookings row (which is what
// actually holds the room via the EXCLUDE constraint) would drift out of
// sync with the event's own date/time otherwise, and re-validating a full
// room move is a materially bigger scope item than "let the organiser fix
// a typo or open up more seats." Cancel + rebook covers a genuine
// date/time change, same as every other hub's "no edit-through-a-clash"
// convention.
//
// actorId must be this event's own coordinator (the organiser -- assigned
// automatically at promote time) or an admin; no space_owners gate, same
// as the rest of this deliberately-open hub.
export async function updateSpaceEvent(db, eventId, actorId, isAdmin, fields) {
  const { data: event, error: fetchErr } = await db
    .from('events')
    .select('id, hub_type, title, max_seats, max_seats_per_booking')
    .eq('id', eventId).eq('archived', false).maybeSingle()
  if (fetchErr) throw new Error(`Could not load event: ${fetchErr.message}`)
  if (!event || event.hub_type !== 'space') return { error: 'Space event not found', status: 404 }

  if (!isAdmin) {
    const { data: coord } = await db
      .from('event_coordinators')
      .select('id').eq('event_id', eventId).eq('member_id', actorId).is('replaced_at', null).maybeSingle()
    if (!coord) return { error: "Only this event's organiser can edit it", status: 403 }
  }

  const validationError = validatePromoteToEvent(fields)
  if (validationError) return { error: validationError, status: 400 }

  const perBooking = fields.max_seats_per_booking == null || fields.max_seats_per_booking === ''
    ? 4 : Number(fields.max_seats_per_booking)

  const payload = {
    title: fields.title.trim(),
    max_seats: Number(fields.max_seats),
    max_seats_per_booking: perBooking,
    allow_nonresident_guests: perBooking > 1 ? !!fields.allow_nonresident_guests : false,
    require_attendee_names: perBooking > 1 ? !!fields.require_attendee_names : false,
  }

  const changed = payload.title !== event.title
    || payload.max_seats !== event.max_seats
    || payload.max_seats_per_booking !== event.max_seats_per_booking

  const { error: updateErr } = await db.from('events').update(payload).eq('id', eventId)
  if (updateErr) throw new Error(`Could not update event: ${updateErr.message}`)

  // NOTE: promoteWaitlist() is deliberately NOT called from here.
  // lib/spaceBookings.js is imported by components/SpaceBookingForm.js (a
  // client component), so it must stay free of server-only imports --
  // promoteWaitlist.js pulls in lib/push.js (web-push, Node 'net'/'tls'),
  // which broke the client build when tried here (2026-09-18). The caller,
  // app/api/spaces/route.js's PATCH update_space_event action, calls
  // promoteWaitlist(eventId) itself after this returns instead -- same
  // BUG-060/BUG-061-class fix (Total Seats increase never re-checked the
  // waitlist), just kept server-side.
  return { event_id: eventId, changed }
}
