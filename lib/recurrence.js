...
export function generateOccurrences(series, opts = {}) {
  ...
  if (series.rule_type === "weekly") {
    ...
    // Guard: an invalid/absent weekday (e.g. config left over from switching rule
    // type in the UI) must NOT spin forever — dow(x) !== NaN is always true.
    if (!Number.isInteger(wd) || wd < 0 || wd > 6) return []
    // Anchor to the first matching weekday on/after start_date (bounded: a weekday
    // always occurs within 7 days), then every 14 days.
    let anchor = start
    for (let i = 0; i < 7 && dow(anchor) !== wd; i++) anchor = addDays(anchor, 1)
    ...
    if (!Number.isInteger(wd) || wd < 0 || wd > 6) return []
    if (ord !== "last" && !(Number.isInteger(ord) && ord >= 1 && ord <= 4)) return []
    let [y, m] = [from.getUTCFullYear(), from.getUTCMonth() + 1]
    for (let i = 0; i <= months + 1 && out.length < cap; i++) {
