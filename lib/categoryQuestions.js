...
// category answerer is a role, not a licence to unmask. display_name (2026-08-14)
// rides ahead of the real name whenever the member isn't masked -- see
// lib/memberName.js for the shared rule this delegates to.
export function displayRecipientName(member, viewer) {
    if (!member) return null
        return resolveMemberName(member, { viewerId: viewer?.id, canManage: !!viewer?.is_admin })
}
§──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────§────────────────────────────────────────// Pure decision logic for "Ask a Contact Category" (scope §3).
//
// Deliberately separated from lib/questionRouting.js: that file does the
