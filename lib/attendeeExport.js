...
// Partial Grouped and Refunds last."). Same zero-dependency window.print()
// convention as exportAttendeeListPdf above -- a dedicated print-only
// window built from the reconciliation data, not the live page.
//
...
// reconciliation summary. `colour` here must be a literal CSS colour (e.g.
// "#166534"), never a var(--token) -- this print window is a fresh
...
export function exportPaymentReconciliationPdf({ eventTitle, eventSubtitle, summaryLines, groups }) {
  const esc = (s) => String(s ?? "").replace(/[&<>"']/g, (c) => (
    { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]
  ))
