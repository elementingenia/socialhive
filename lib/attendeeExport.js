...
// in the exported PDF.
//
// UPDATED 2026-09-16 (Iain): once the payment-reconciliation export's popup
// was replaced with the shared in-app DocumentViewer (see that function's
// header comment below and app/(app)/documents/view/page.js), Iain: "Once
// merged and verified, lets fix all the other places with the old bare-popup
// print only and bring them in line." This was the other bare
// `window.open("", "_blank", ...)` + print-button popup in the app -- same
// "no way back", same real popup-blocker exposure -- so it gets the
// identical treatment: build the same report HTML, wrap it in a Blob, and
// hand it to the shared DocumentViewer via a normal client-side route change
// instead of a new window/tab. Requires a Next.js `router` (from
// next/navigation's useRouter()) for the same reason exportPaymentReconciliationPdf
// does -- this is now same-document SPA navigation, and the Blob URL only
// stays valid because the document that created it is never unloaded.
// `hubColour` tints the viewer's Close button to match the hub/club this
// attendee list came from (e.g. "var(--teal)", or clubColour(club) for
// Clubs) -- a var(--token) is fine here since it only decorates the real
// app's own toolbar, which has the app's CSS custom properties in scope.
//
// FIXED 2026-09-17 (Iain): reported rendering in dark mode inside the
// DocumentViewer iframe, but correctly light when printed. Root cause
// confirmed by inspection -- this generated HTML is a bare standalone
// document with no <meta name="color-scheme"> and no explicit
// background-color, so a device/browser with automatic dark theming
// (which only reads author colour-scheme intent, not print output) was
// free to invert it on screen; the print path bypasses that heuristic
// entirely, which is why print already looked right. Added an explicit
// `color-scheme: light only` meta and a literal white background so the
// report can't be auto-darkened. Same fix applied to
// exportPaymentReconciliationPdf below -- both are the only bare
// generated-HTML documents in the app (every other export/print surface
// prints the live app page itself, which already has explicit light
// colours throughout).
//
// sections: [{ heading, rows: [{ name, seats, note }] }]
export function exportAttendeeListPdf({ eventTitle, eventSubtitle, sections, router, hubColour }) {
...
<style>
  * { box-sizing: border-box; }
  ...
  body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif; color: #111827; background: #ffffff; padding: 32px; margin: 0; }
...
<body>
  ...
  const fileName = `${eventTitle || "Event"} - Attendees.html`
