...
const PUSH_TYPES = new Set([
  ...
  "voting_opened",
  "committee_post_added",
  "survey_opened",
  "survey_results_published",
  // Personal vehicle offers (migration 109, Iain 2026-09-17): a driver
  // bumping an already-seated passenger (as distinct from that passenger
  // leaving on their own, which needs no notification at all) is the one
  // mandatory notification touchpoint the scope called for -- "a real
  // notification, not a silent removal." Pushed for the same reason
  // event_cancelled is: it's the highest-stakes type in this feature, and a
  // resident who doesn't happen to open the bell drawer would otherwise
  // never learn their seat was taken away. The seat-assigned/seat-claimed
  // courtesy notifications (app/api/vehicle-offers/route.js) are
  // deliberately NOT pushed -- lower stakes, in-app only, same precedent as
  // bar_reconciled just above.
  "vehicle_offer_seat_removed",
])
...
const PUSH_TITLES = {
  ...
  voting_opened:          "Voting is open",
  space_request_validate: "Confirm with Ingenia",
  committee_post_added:   "Committee update",
  survey_opened:          "New survey",
  survey_results_published: "Survey results are in",
  vehicle_offer_seat_removed: "Car seat no longer available",
}
