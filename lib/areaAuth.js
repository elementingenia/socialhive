...
// Admin OR this hub's Owner OR the specific survey's own assigned
// Coordinator (surveys.coordinator_id) -- Surveys hub (099_surveys.sql),
// mirrors requireVotingEventManage()'s exact shape above (same reasoning:
// surveys is its own table, no hub_type/club_id, no event_coordinators join
// row, coordinator is a single nullable FK on the row itself). Used for
// managing a survey (create/edit/publish/close) -- NOT for viewing its
// results, which is narrower still (Change Request #3, 2026-09-08:
// coordinator-only, not Admin/Owner too -- see lib/voting.js's
// canSeeResults(), reused as-is for Surveys' results gate rather than
// duplicated).
export async function requireSurveyManage(req, surveyId) {
  ...
  const { data: survey } = await supa.from("surveys").select("id, coordinator_id").eq("id", surveyId).maybeSingle()
  if (!survey) return { error: "Survey not found", status: 404 }
  if (survey.coordinator_id && survey.coordinator_id === member.id) return { member }

  return { error: "Admins, Surveys Owners, or this survey's coordinator only", status: 403 }
}
