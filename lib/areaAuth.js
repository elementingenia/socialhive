...
// Admin OR this hub's Owner OR one of the specific survey's own assigned
// Coordinators (survey_coordinators, 104_survey_coordinators.sql) -- Surveys
// hub (099_surveys.sql). Originally a single nullable coordinator_id column
// (mirroring requireVotingEventManage()'s shape), widened to a join table
// per Iain (2026-09-09, "Need to be able to add more than one coordinator"):
// "follow same pattern as other hubs / Groups and clubs" -- that pattern is
// event_coordinators, i.e. any number of members, all with identical
// permissions, no "primary". Used for managing a survey (create/edit/
// publish/close) -- NOT for viewing its results, which is narrower still
// (Change Request #3, 2026-09-08: coordinator-only, not Admin/Owner too --
// see lib/voting.js's canSeeResults(), reused as-is for Surveys' results
// gate rather than duplicated).
export async function requireSurveyManage(req, surveyId) {
  const { error, status, member } = await resolveMember(req)
  if (error) return { error, status }
  if (member.is_admin) return { member }
  if (await isAreaOwner(member.id, "hub", "surveys")) return { member }

  const { data: survey } = await supa.from("surveys").select("id").eq("id", surveyId).maybeSingle()
  if (!survey) return { error: "Survey not found", status: 404 }
  const { data: coord } = await supa.from("survey_coordinators")
    .select("id").eq("survey_id", surveyId).eq("member_id", member.id).is("replaced_at", null).maybeSingle()
  if (coord) return { member }

  return { error: "Admins, Surveys Owners, or this survey's coordinators only", status: 403 }
}
