...
/**
 * Convert a timestamptz instant into Sydney LOCAL wall-clock 'YYYY-MM-DD'.
 * Added 2026-08-22 for the Book a Space "promote to event" bridge
 * (lib/spaceBookings.js) -- a space_bookings row only carries starts_at/
 * ends_at, but events.event_date needs a plain local date string. Mirrors
 * isoToSydneyHHMM's approach exactly (en-CA gives YYYY-MM-DD with no
 * reformatting, same as sydneyTodayStr above) rather than inventing a new
 * conversion technique.
 */
export function isoToSydneyDateStr (iso) {
  if (!iso) return ''
...
/**
 * 'YYYY-MM-DD' N days from an arbitrary given date string -- pure calendar
 * arithmetic on plain Y/M/D integers via Date.UTC, never the real "now" or
 * a real timezone, same exemption class as lib/recurrence.js's own date
 * math. Added 2026-09-15 (Iain, Book Club return-date-to-next-event
 * feature) so components/ClubHome.js's isoPlusOneDay didn't need its own
 * local copy of this UTC-pure logic living outside lib/date.js.
 */
export function dateStrPlusDays (dateStr, days) {
