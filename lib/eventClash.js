...
// The location join is additive (Iain, 2026-08-04's space-booking clash list
// needs "the space booked" per clashing item) -- the three existing callers
// (screenings/social/clubs event forms) only ever read `.title`/`.hub_type`
// and ignore the extra field, so this doesn't change their behaviour.
export async function findSameDateEvents(db, { event_date, exclude_event_id }) {
  if (!event_date) return []
  let q = db.from("events")
    .select("id, title, hub_type, locations(name)")
...
export async function fetchLocation(db, locationId) {
  ...
  const { data, error } = await db
    .from("locations").select("id, name, bookable, request_only").eq("id", locationId).maybeSingle()
...
export async function resolveLocationByName(db, locationName) {
  ...
  const { data, error } = await db
    .from("locations").select("id, name, bookable, request_only").eq("name", locationName).maybeSingle()
...
// Exported so lib/spaceBookings.js can format space_bookings' timestamptz
// windows with the same wording as this file's event-window messages, rather
// than growing a second copy of the same 12-line formatter.
