...
// Second-pass shrink for the Happenings News archive cron (see
// app/api/cron/happenings-news-archive/route.js) -- once a post ages past
// its hub's admin-configured archive delay (30/90/150 days), its photos are
// re-encoded down to a small thumbnail and the ORIGINAL replaced in place
// (same storage_path, same URL -- nothing else needs to change), per Iain
// 2026-09-21: "Archive means reduce images to min possible size... and
// retain text." Deliberately a separate function rather than a parameter on
// resizeImage() above -- the two have different failure-handling needs (this
// one runs unattended from a cron with no resident waiting on a response,
// so a thrown isImageProcessingError here should be caught and skipped by
// the caller, not surfaced as a user-facing message).
export async function shrinkImageForArchive(buffer) {
  const out = await sharp(buffer)
    .rotate()
    .resize({ width: ARCHIVE_MAX_DIMENSION, height: ARCHIVE_MAX_DIMENSION, fit: "inside", withoutEnlargement: true })
    .webp({ quality: ARCHIVE_WEBP_QUALITY })
    .toBuffer()
  return { buffer: out, contentType: "image/webp" }
}

const ARCHIVE_MAX_DIMENSION = 200
const ARCHIVE_WEBP_QUALITY = 45
